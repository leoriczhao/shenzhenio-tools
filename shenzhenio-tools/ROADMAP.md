...
- 为全部内置谜题生成 `BoardSpec`，无法提取字段显式标为 unknown。
...
1. 明确坐标、占用格、接触格和 Trace graph 的唯一语义。
2. 支持用户固定位置和 route hints 的确定性布线。
3. 实现单网络 Lee/A* router。
4. 实现多端点网络、障碍、已有线复用和回溯。
5. 实现多网络协商拥塞与确定性重试。
6. 加入桥接器和允许跨线的器件模型。
...
1. 器件接触引脚和 Board terminal 坐标必须可靠。
2. 游戏事件调度和 XBus 时序必须通过差分实验确认。

因此当前不能优先投入完整自动布局或高级语言编译器；它们依赖的数据和判定器尚未稳定。
...
- 生成 `Sz035`，与当前手工 BoardSpec 做结构差异。

完成后即可决定是否能批量覆盖所有官方开发板。
