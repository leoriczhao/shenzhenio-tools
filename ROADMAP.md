...
| Python 电路 API | 可放置器件、连网，并生成类型化 ProgramIR | 需要源码位置映射和高级结构化 API |
| 器件库 | 66 个 `ChipType` 均可生成运行时 `PartSpec/Part`；3 个手册基准已验证 | 需要补全行为分类并生成稳定命名类 |
| 开发板库 | 50 个 `BoardSpec` 均可生成运行时 `Board`，包含 provided part、terminal 绑定和初始线 | 需要补全特殊显示/桥接行为与版本化内置快照 |
| 静态检查 | 已检查放置边界、terminal/器件碰撞、线路合法性和 API/物理网络一致性 | 缺更多特殊器件、驱动冲突和完整代码规则 |
| 自定义谜题 | 可提取 Lua 板面和 terminal 元数据 | 缺完整语义和安全执行模型 |
| 布局/布线 | 已有约束 placer、联合布线评分、Lee 多网 router、协商拥塞、顺序回溯、初始线复用、route hints 和 bridge | 缺更强的局部回溯和大规模基准 |
| 仿真器 | MCxxxx VM、多芯片调度器、Simple 和基础 XBus 已实现 | 需要外设和游戏差分 |
| Testbench | 未实现 | 需要事件流、波形断言和官方差分样例 |
| 游戏闭环 | 已有进程检查和 install 雏形 | 需要批量验证和结果记录 |

当前回归基线：85 个单元测试通过；`Sz035` 示例可从无坐标 `UC6` 自动布局、自动布线，并已在官方游戏中完成全部测试。游戏回写的电路、Trace 和代码与构建产物一致，只增加 `production-cost=500`、`power-usage=280`、`lines-of-code=10` 三项成绩。该方案现可在外部仿真中接收 `radio-rx` 包：空输入按 non-blocking XBus 得到 `-999`，输入 1 得到 `acc=100`、蜂鸣器 100 和功耗 7。
...
状态：进行中。规范目录和运行时目录驱动均已落地，行为层与内置快照尚未完成。
...
- 自动生成器件 Python 类，保留 `MC6000()`、`cpu.x0` 这种 API。（通用 `Part` 已目录驱动，稳定命名类待生成）
- [x] 为全部内置谜题生成 `BoardSpec`，无法提取字段显式标为 unknown。
- [x] 让全部 50 个 `BoardSpec` 可构造运行时 `Board`。
- [x] 将当前手填 `parts.py`、`boards.py` 降级为验证夹具或手册语义 override。
...
- [x] 将 `ProgramBuilder` 改为类型化 `ProgramIR`，最后一步才渲染文本。
...
状态：进行中。坐标模型、route hints、Lee、多端点树、协商拥塞、bridge 和约束布局器已实现。

任务按顺序推进：

1. [x] 明确坐标、占用格、接触格和 Trace graph 的唯一语义。
2. [x] 支持用户固定位置和 route hints 的确定性布线。
3. [x] 实现单网络 Lee router。
4. 实现多端点网络、障碍、已有线复用和回溯。（多端点树、引脚障碍、初始线复用已完成）
5. [x] 实现多网络协商拥塞与确定性重试。
6. [x] 加入桥接器和允许跨线的器件模型。
7. [x] 实现约束布局器，并以可布通性而非仅曼哈顿距离评分。
8. [x] 联合布局与布线搜索，提供超时、状态预算和最优已知结果。
...
状态：进行中。类型化解析、芯片级校验、MCxxxx VM、公平逐 tick 调度、Simple 网络和基础 XBus rendezvous 已完成；外设与游戏差分尚未实现。

任务：

- [x] 实现 MC4000、MC4000X、MC6000 共用的指令和寄存器执行核心。
- [x] 实现标签、`+/-` 条件、局部睡眠/阻塞状态和功耗统计。
- 核对存档 corpus 与游戏 parser 是否存在手册外语法；当前手册和已扫描存档均未发现先前计划中假设的 `@` 前缀。
- [x] 定义单 tick 内所有芯片的确定性调度阶段。（跨芯片顺序待游戏差分）
- [x] 实现 Simple 网络持续电平、输入/输出模式和多节点组合规则。（最大值组合待游戏差分）
- [x] 实现 XBus 主动/被动收发、阻塞、non-blocking terminal 和单包仲裁。（多候选 tie-break 待游戏差分）
